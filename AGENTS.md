...
- 推荐定额 L3 正式编号不得使用宿主右键菜单/批量粘贴作为写入路径；已验证该路径可返回却不生成定额行。2024 实机进一步确认 `dataGridViewDE` 是自定义 `RecoNet.DataGridViewDe`，表格、定额编号列和单元格均为只读，`BeginEdit(true)` 返回 false 且不会产生 `EditingControl`，因此也不得把普通 `DataGridView` 的 `BeginEdit` + `TextBoxBase` 当作原生输入路径。下一次修复前必须先记录一次用户在宿主中手工成功输入正式定额的实际命令/事件链及数据库新增行，再复用自定义 `ProcessCmdKey` 或明确的宿主业务入口；不得继续部署未经实机证据支持的猜测路径。最终仍只能定位末尾空白业务行，并以项目数据库新增行的完整身份和数量作成功判定。
