...
- 组件框中的条目证据必须保存到每个目标；条目证据来自写入时用户选定的当前项目真实条目，`SF` 使用自动改道后的设备购置费条目。所有普通、材料和纯辅助目标都按自身持久化的目标级条目证据形成 `EngineeringTemplate` 专业范围，不再用主定额跟随或工程前缀投票扩展范围。推荐学习库“未归类”只由持久化 `EngineeringTemplate` 是否存在决定，本机临时上下文不得把框移出“未归类”。
...
- 推荐定额同步宿主“当前条目”时，`Tv_tree.SelectedNode` 有值就以界面真实选中项为准，不得要求它与宿主 `CurrNode` 引用相同；宿主切换焦点期间 `SelectedNode` 可能暂时为空，此时允许回退 `CurrNode`，但仍须通过叶节点、可编辑定额表、条目序号/编号、项目 `章节表` 唯一身份和项目连接校验。若已选中叶条目且重新激活推荐窗口后仍显示默认提示，先区分 `SelectedNode` 暂空、事件未触发和字段读取失败，不要只按对象引用推断。
