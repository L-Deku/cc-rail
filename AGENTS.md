...
- 推荐定额写入路径必须保留 `Smart fill apply` 结构化日志：begin / plan / ok / blocked / failed / rejected 覆盖界面早退、整组阻断、事务失败与成功；日志只记单元、条目、层级、序号与计数，不记工程量名、公式和价格。清理或重构写入链时不得连带删除这些日志。
...
- 推荐定额右键绑定的软件选中行包含 `SF` 且当前组件已有目标时，必须让用户选择“替换”或“补充”：替换沿用整组替换；补充保留原目标并加入按完整身份去重后的 SF，将合并后的完整组件作为 correction 回流，并把此前缺 SF 的旧组件作为 rejection 回流。当前组件为空或普通定额右键绑定时不增加该选择步骤。
