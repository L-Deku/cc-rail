...
- 广材助手桥接程序 `GuangcaiBridge.exe` 需要按 x86 构建；引用 UI Automation 时优先从 .NET GAC 查找 `UIAutomationClient.dll`、`UIAutomationTypes.dll` 和 `WindowsBase.dll`，不要假定框架目录下存在短路径引用。
- 铁路主程序运行时会锁住已加载的插件 DLL；如果只修改 `GuangcaiBridge.exe`，可在桥接进程退出后单独替换该 exe，不必等待铁路主程序释放全部插件 DLL。
