...
- 名字驱动预览把工程量名跨组件成员行合并绘制时，应由当前正在绘制的成员单元格作锚点，按组内各行高度反推出同一个稳定整组矩形，再分别裁剪绘制；不得依赖未显示组首/组尾的 `GetCellDisplayRectangle` 或各行瞬时 `Displayed` 状态。选择变化和滚动事件都必须使合并工程量名整列或完整组失效重绘，并用“滚动多行组件无空白无重叠、点击后续成员行首行文字仍完整”做现场验收。
