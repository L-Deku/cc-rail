...
- `RecoQuotaRecommend` 的消耗密文桥每 3 秒检查 `RecoQuotaData\consume-encrypt-requests.tsv`，且"已处理签名"只存在进程内存里，宿主每次启动都会把该文件整个重处理一遍（6 月迁移留下的 7386 行请求约让界面线程卡数秒）。2026-09-23 已按用户决定把 6 月的请求/响应文件归档到 `RecoQuotaData\archive-consume-crypto-bridge-2026-06\`（只移动不删除）；迁移结束后不得把请求文件放回 `RecoQuotaData` 根目录，再次需要密文时由迁移工具重新生成请求。
...
- 推荐定额右键绑定的软件选中行包含 `SF` 且当前组件已有目标时，必须让用户选择“替换”或“补充”：替换沿用整组替换；补充保留原目标并加入按完整身份去重后的 SF，将合并后的完整组件作为 correction 回流，并把此前缺 SF 的旧组件作为 rejection 回流。对话框出现前必须用与写入路径共用的 SF 双向约束分别预检“替换结果”和“补充结果”，不可写的选项必须禁用并显示原因，两者都不可写时只允许取消；绑定后立即重算 SF 界面阻断状态。当前组件为空或普通定额右键绑定时不增加该选择步骤。SF 行只在设备购置费条目的表格里选得到，所以站在名称含“设备购置费”的条目上右键补充且本次所选行全是 SF 时，“补充结果”在当前条目预检失败后必须再以旧组件普通定额自带的条目证据（`ChosenItemSeq`/`ChosenItemNo`，须唯一）为参照条目重新预检，并要求从该条目写入时 SF 改道结果正是当前设备购置费条目；通过则启用“补充”并提示写入时站回参照条目，旧组件缺证据、证据不唯一或本次夹带普通定额时维持禁用并说明原因。绑定后该组在当前设备购置费条目下仍显示阻断，切到参照条目后自动解除，这是预期行为。
...
- 推荐数量的默认单位族必须保留这些业务别名：`延米/延长米/双延米/横延米/单侧米/双侧米/延米桥长` 归入 `m`；`顶平米/投影面积m2/处理面积m2/单层10m2` 归入 `m2`（其中 `单层10m2` 自带 10 倍量级）；`m3湿土/m3土/m3混凝土/m3空间` 归入 `m3`；`个/件/台/套/组/座/孔/次/顶次/项次` 归入离散计数维度并保留 `10/100/1000` 等前缀量级；`亩/公顷` 与 `m2` 使用精确面积比例（`1亩=2000/3m2`、`1公顷=10000m2`）。`天然密实方/压实方` 等不同方态仍视为不同业务量纲，未经人工绑定不得静默换算。
- 推荐定额窗口中，只有用户实际修改某行数量并右键选择“绑定当前数量为单位关系系数”时，才允许把本次比例保存为 `V0` 系数公式；右键菜单判断可用状态前必须先结束并同步数量单元格编辑，`1` 改为 `1*1` 等用户明确确认的跨单位 1:1 关系也应保存为 `V0`，不得因计算结果仍为 1 禁用入口。作用域严格为“同一归一化工程量名称 + 同一定额完整身份（目标类型、完整编号、规范化名称、规范化单位）”，并继续受当前软件分区、办法文号和条目专业范围约束。人工系数仅在该完整身份命中时优先于普通历史公式和默认单位换算，不得跨名称或跨定额身份复用；部署读取该能力的 DLL 前必须先幂等增加 `QuantityFormulaRule.target_name` 与 `manual_override` 字段。
...
- 自动/批量绑定重新生成完整组件时，如果同一软件分区、办法、项目、工程量签名、工作簿、工作表和来源行已有目标集合为新组件严格子集的正权重历史组件，应在同一个 RecoLearning SQL 事务中把新完整组件记为 correction、旧缺项子集记为 rejection；不得降权非子集备选。推荐预览有两套以上有效完整组件时，即使最高分组件可自动勾选，也必须在“源行定额”列保留全部组件下拉供人工切换。
...
- 工程量名称拆字段口径（2026-09-11 起）：与数量单位等值的片段一律不进主名称（不只末位）；序号片段（`1）`、`①`、`Ⅰ`、`1.1.2`、单字母等，整格完整匹配 `IsRowNameOrdinalFragment`）与章节锚点、纵向合并片段一样只进 `ContextLabel`，存但不参与匹配。章节锚点与序号解析只有 `TryGetChapterOrdinal` 一个规则源：接受阿拉伯与汉字序号，无「第」前缀时章/节/部分之后必须是空白、顿号或串尾（`四节段预制梁`、`二分之一预制块` 不是锚点），序号相等时剥掉序号只比标题。名称评分前 `NormalizeQuantityMatchName` 必须剥掉尾部括号单位，`ExtractMatchNumbers` 不得把单位幂次和串尾"数字+单位"当规格数字，否则 `C30混凝土(m3)` 与 `C25混凝土(m3)` 会越过 55 分被自动认领。检索侧 `QuotaMatchesRequiredPhrase` 原文不命中时须去括号再比一次（`警示（示踪）带铺设` 必须命中 `警示带`）；`RecoQuotaRecommend/verify-samples.ps1` 已是断言式回归，改检索规则后必须跑通。
- 绑定存储 `ExcelLinkStore` 只能临时文件 + `File.Replace` 原子保存；加载失败时把损坏文件改名 `.corrupt-<时间>.xml` 备份并标记 `LoadFailed`，该状态下保存必须拒绝并提示，不得用空库覆盖历史绑定。批量绑定的 Excel 地址按所选行序号推进，失败行同样占位并在结果里列出。学习库入批前剔除不可推荐组单独记序号日志，只有全部可审计组证据无效才判永久失败；推荐预览行键与公式操作数键用 `BuildSmartRowQuantitySignature` 与 SQL 侧同口径剥单位。
...
- 推荐定额点击“写入当前条目”后不再弹写入前确认框，也不得隐藏、关闭或把焦点强制交给宿主窗口；写入完成后仍显示结果确定窗口。写入成功后必须保留原预览行、勾选状态和滚动位置，方便继续选择其他组写入别的条目；不得把“写入成功”当作数据源变化而调用预览失效。事务开始前仍须用与预览相同的 `ResolveChapterNo` 链重新核对当前 `SelectedNode`、条目身份、项目连接对象和 `DataSource|Database`，任一变化都拒绝旧计划。
- 推荐定额的一次预览只允许在预览时的原单元内复用，可切换该单元内的不同条目；宿主当前单元变化时必须提示“项目或当前单元已切换，请重新预览”并拒绝旧预览写入。`SmartPreviewContext.CurrentUnitId` / `CurrentUnitCode` 与 `IsSmartPreviewContextCurrent` 中的两项比对是安全门禁，不得删除；跨单元写入必须在新单元重新预览。
...
- 名字驱动预览把工程量名跨组件成员行合并绘制时，应由当前正在绘制的成员单元格作锚点，按组内各行高度反推出同一个稳定整组矩形，再分别裁剪绘制；不得依赖未显示组首/组尾的 `GetCellDisplayRectangle` 或各行瞬时 `Displayed` 状态。选择变化和滚动事件都必须使合并工程量名整列或完整组失效重绘，并用“滚动多行组件无空白无重叠、点击后续成员行首行文字仍完整”做现场验收。合并名称矩形不得裁到可见数据区，也不得以任何方式依赖滚动位置或 `ClientRectangle`：DataGridView 垂直滚动只位块搬移旧像素并重绘新暴露行，只有绘制结果与滚动偏移无关，`tname` 列才能与其他列同一帧移动。靠 `Scroll` 事件失效重绘补救的两种做法 2026-09-01 都在现场实测被否决——异步 `InvalidateColumn` 让该列比其他列晚一拍，同步 `Update()`/`Refresh()` 会卡；因此不得再加 `Scroll` 处理器。组首滚出表头后名称随组滚出属于预期行为（与 Excel 合并单元格一致）。回归测试应断言“滚动事件返回前无同步绘制、滚动只重绘新暴露行、同一组相对锚点单元格的偏移与高度不随滚动位置变化、组首滚出表头后矩形不被裁到可见区”，不得用源码正则判断处理器是否存在。
...
- 补充材料（`RecoNet.补充单价.FormBcCl`）与补充设备（`RecoNet.补充单价.FormBcSb`）窗口的“批量删除”按钮是独立插件 `RecoSupplementBulkDelete.dll`（源码 `RecoSupplementBulkDelete/`，日志 `RecoSupplementBulkDelete.log`），由 `RecoPluginLoader` 按文件名加载、`RecoQuotaRecommend/build.ps1` 一并编译部署，不得再并入 `RecoExpandPanel.dll`（2026-09-20 用户要求恢复原样）。窗体检测挂在 `Application.Idle` 上按索引读 `Application.OpenForms`（不分配、不扫控件树），不得再加回 500ms 之类的常驻定时器：2026-09-23 用户反馈装插件后打开料费方案变卡，定时器每次唤醒消息循环都会连带触发进程内全部 Idle 处理器，是本插件唯一常驻动作，已移除。删除只用窗体自身 `m_cnn`，目标表从 `m_sql` 的 `from` 后解析（项目库 `材料单价库`/设备表），按电算代号+名称列执行；补充材料代号必须在 4 开头 9 位号段（状态栏 `m_start/m_end` 只是当前设计单位子区间，不能作守卫）。引用守卫：查一次 `information_schema` 找同库所有带 `电算代号`/`定额编号` 列的基表并缓存，每表每批一条 `in(...)` 查询，任一表引用即跳过并在结果里点名（项目库没有 `定额库消耗`，早期只查它的版本实测无效）；窗口只读或“添加”按钮不可用时拒绝。宿主方法体受运行时保护，2024 版虽有 `Btn_BulkDelete_Click` 方法但无对应按钮，不得盲调。删除后先从已绑定 DataTable 本地移除，失败才回退宿主 `comboBox_SelectedIndexChanged` 重查。
...
- 用户明确要求“直接从软件导出”时，必须通过目标软件自身的原生导出功能生成文件；未经用户另行明确同意，禁止改用数据库查询、脚本生成、模板复制或 Excel/WPS 重保存，也不得修改原生导出文件。
- 软件界面导出未正常完成、文件数量或路径异常、导出格式与预期不一致、打印预览缺少页眉页脚/重复标题行/表格边框，或执行中需要改用任何替代路线时，必须立即停止并向用户说明异常、影响和拟议方案；未获得用户明确同意，不得继续替代生成或覆盖目标文件。
- 原生 Excel 导出的验收不得只核对单元格值或文件哈希；至少检查页眉页脚、重复打印标题行、打印网格线或表格边框以及 PDF/打印预览。批量任务先原生导出一个站的一组三表供用户确认，通过后再继续批量处理。
