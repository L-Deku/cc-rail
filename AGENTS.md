...
- 推荐定额右键绑定的软件选中行包含 `SF` 且当前组件已有目标时，必须让用户选择“替换”或“补充”：替换沿用整组替换；补充保留原目标并加入按完整身份去重后的 SF，将合并后的完整组件作为 correction 回流，并把此前缺 SF 的旧组件作为 rejection 回流。对话框出现前必须用与写入路径共用的 SF 双向约束分别预检“替换结果”和“补充结果”，不可写的选项必须禁用并显示原因，两者都不可写时只允许取消；绑定后立即重算 SF 界面阻断状态。当前组件为空或普通定额右键绑定时不增加该选择步骤。
