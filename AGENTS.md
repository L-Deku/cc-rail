...
- 名字驱动预览把工程量名跨组件成员行合并绘制时，应由当前正在绘制的成员单元格作锚点，按组内各行高度反推出同一个稳定整组矩形，再分别裁剪绘制；不得依赖未显示组首/组尾的 `GetCellDisplayRectangle` 或各行瞬时 `Displayed` 状态。选择变化和滚动事件都必须使合并工程量名整列或完整组失效重绘，并用“滚动多行组件无空白无重叠、点击后续成员行首行文字仍完整”做现场验收。合并名称矩形对可见数据区做裁剪与“滚动时整列失效”是绑定的一对：裁剪使绘制结果依赖滚动位置，DataGridView 的位块搬移不再成立，必须靠滚动失效补齐；删掉其中任何一方都会立刻重现滚动空白/重影或滚动卡顿，不得单独取舍。滚动事件只允许异步 `InvalidateColumn`，不得调用 `Update()`/`Refresh()` 强制同步绘制——“事件返回前完成绘制”只有不跑消息泵的测试能观测到，不是现场需求；回归测试应断言“滚动后当前所有可见行都被重绘过”，不得用源码正则判断处理器是否存在。
