...
- 推荐定额同步宿主“当前条目”时，只要 `Tv_tree` 存在就以界面真实的 `SelectedNode` 为准，只有拿不到树控件时才回退 `CurrNode`；宿主的 `CurrNode` 可能是陈旧或不同对象，不得要求它与 `SelectedNode` 引用相同。安全性继续由叶节点、可编辑定额表、条目序号/编号、项目 `章节表` 唯一身份和项目连接校验保证。若用户已选中叶条目而窗口仍显示默认“请选择条目”，先检查选中事件和最前段节点读取门禁，不要误判为章节表或写入失败。
