...
Assert-Contains $excelLink 'List<ExcelQuotaLink> pendingLinks = new List<ExcelQuotaLink>();' 'The auto-match save callback does not collect accepted links for one shared name finalization.'
Assert-Contains $excelLink 'Dictionary<ExcelQuotaLink, string> savedQuantityNames = FinalizeBoundLinkNames(pendingLinks, false);' 'The auto-match save callback does not derive learning names from the shared main-name reader.'
Assert-NotContains $excelLink 'savedQuantityNames[item.Link] = item.QuantityName ?? "";' 'The auto-match save callback still sends the legacy preview name directly to learning.'
Assert-Contains $excelLink 'displayFallbackNames.TryGetValue(link, out displayFallback)' 'The auto-match UI no longer preserves its display-only fallback when workbook reread fails.'
