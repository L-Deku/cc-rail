...
namespace RecoNet
{
    public partial class FormPanel : Form
    {
        ...
        private static readonly object ExcelLinkKnownUnitLock = new object();
        private static HashSet<string> ExcelLinkKnownUnits;
        ...
        private static string StripExcelLinkCubicUnitQualifier(string unit)
        {
            if (String.IsNullOrEmpty(unit))
            {
                return "";
            ...
            for (int i = 0; i < prefixes.Length; i++)
            {
                ...
                if (String.Equals(unit, prefix, StringComparison.Ordinal))
                {
                    return "m3";
                }

                if (unit.StartsWith(prefix, StringComparison.Ordinal) && unit.Length > prefix.Length)
                {
                    string rest = unit.Substring(prefix.Length);
                    if (StartsWithExcelLinkScaleOrMetricUnit(rest))
                    {
                        return rest;
                    }
                }
        ...
        private static bool StartsWithExcelLinkScaleOrMetricUnit(string text)
        {
            ...
            return Char.IsDigit(first) ||
                   first == '.' ||
                   first == '\u4e07' ||
                   text.StartsWith("m3", StringComparison.Ordinal);
        }
        ...
        private static bool IsExcelLinkSimpleBindingBlocked(DataGridViewRow row, ExcelCellAddress cell, out string excelUnit, out string quotaUnit)
        {
        ...
        private static string BuildSimpleBindingUnitMismatchMessage(string excelUnit, string quotaUnit)
        {
            if (String.IsNullOrWhiteSpace(excelUnit) || String.IsNullOrWhiteSpace(quotaUnit))
            {
                return "\u5355\u4f4d\u672a\u8bc6\u522b\uff1aExcel " + (String.IsNullOrWhiteSpace(excelUnit) ? "\u672a\u8bc6\u522b" : excelUnit) + " \u2192 \u5b9a\u989d " + (String.IsNullOrWhiteSpace(quotaUnit) ? "\u672a\u8bc6\u522b" : quotaUnit) + "\uff0c\u7b80\u5355\u7ed1\u5b9a\u5df2\u7981\u6b62\u3002\u8bf7\u5207\u6362\u5230\u8868\u8fbe\u5f0f\u7ed1\u5b9a\u624b\u52a8\u5904\u7406\u3002";
            }

            return "\u5355\u4f4d\u4e0d\u4e00\u81f4\uff1aExcel " + excelUnit + " \u2192 \u5b9a\u989d " + quotaUnit + "\uff0c\u7b80\u5355\u7ed1\u5b9a\u5df2\u7981\u6b62\u3002\u8bf7\u5207\u6362\u5230\u8868\u8fbe\u5f0f\u7ed1\u5b9a\u624b\u52a8\u5904\u7406\u3002";
        }
        ...
        private static bool IsExcelLinkCountUnit(string unit)
        {
            ...
            return GetExcelLinkKnownUnits().Contains(NormalizeExcelLinkUnit(unit));
        }

        private static HashSet<string> GetExcelLinkKnownUnits()
        {
            lock (ExcelLinkKnownUnitLock)
            {
                if (ExcelLinkKnownUnits == null)
                {
                    ExcelLinkKnownUnits = BuildExcelLinkKnownUnits();
                }

                return ExcelLinkKnownUnits;
            }
        }

        private static HashSet<string> BuildExcelLinkKnownUnits()
        {
            HashSet<string> units = new HashSet<string>(StringComparer.Ordinal);
            foreach (string unit in GetDefaultExcelLinkCountUnits())
            {
                AddExcelLinkKnownUnit(units, unit);
            }
            ...
            try
            {
                ...
                {
                    string unitPath = Path.Combine(dataDir, ExcelLinkUnitsFileName);
                    if (ShouldRefreshExcelLinkUnitFile(unitPath, dataDir))
                    {
                        SaveExcelLinkUnitsFromIndexes(dataDir, unitPath);
                    }

                    if (File.Exists(unitPath))
                    {
                        foreach (string line in File.ReadAllLines(unitPath, Encoding.UTF8))
                        {
                            AddExcelLinkKnownUnit(units, line);
                        }
                    }
                }
            }
            ...
            {
                Log("Load Excel link units failed: " + ex.Message);
            }

            return units;
        }

        private static bool ShouldRefreshExcelLinkUnitFile(string unitPath, string dataDir)
        {
            if (String.IsNullOrWhiteSpace(unitPath) || !File.Exists(unitPath))
            {
                return true;
            }

            DateTime unitUtc = File.GetLastWriteTimeUtc(unitPath);
            string[] sources = new string[]
            {
                Path.Combine(dataDir, "quota-index.jsonl"),
                Path.Combine(dataDir, "material-index.jsonl")
            };
            foreach (string source in sources)
            {
                if (File.Exists(source) && File.GetLastWriteTimeUtc(source) > unitUtc)
                {
                    return true;
                }
            }
        ...
        private static void SaveExcelLinkUnitsFromIndexes(string dataDir, string unitPath)
        {
            HashSet<string> rawUnits = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (string unit in GetDefaultExcelLinkCountUnits())
            {
                rawUnits.Add(unit);
            }

            CollectExcelLinkUnitsFromIndex(Path.Combine(dataDir, "quota-index.jsonl"), "quota_unit", rawUnits);
            CollectExcelLinkUnitsFromIndex(Path.Combine(dataDir, "material-index.jsonl"), "material_unit", rawUnits);
            if (rawUnits.Count == 0)
            {
                return;
            }
            ...
            File.WriteAllLines(unitPath, rawUnits.OrderBy(u => NormalizeExcelLinkUnit(u), StringComparer.Ordinal).ToArray(), new UTF8Encoding(false));
        }

        private static void CollectExcelLinkUnitsFromIndex(string path, string key, HashSet<string> rawUnits)
        {
            ...
            foreach (string line in File.ReadLines(path, Encoding.UTF8))
            {
                ...
                string unit = GetFlat(row, key).Trim();
                if (!String.IsNullOrWhiteSpace(unit))
                {
                    rawUnits.Add(unit);
                }
        ...
        private static void AddExcelLinkKnownUnit(HashSet<string> units, string unitText)
        {
            if (units == null || String.IsNullOrWhiteSpace(unitText))
            {
                return;
            }
            ...
            decimal factor;
            string baseUnit;
            if (!TryParseExcelLinkUnitPrefix(unitText, out factor, out baseUnit))
            {
                return;
            }
        ...
        private static string[] GetDefaultExcelLinkCountUnits()
        {
            return new string[]
            {
                "\u5904", "\u5ea7", "\u7ec4", "\u6839", "\u9879", "\u53f0", "\u5b54", "\u5957",
        ...
        private static void BindSelectedQuotaToExcel(Form mainForm)
        {
            try
            {
                ...
                string excelUnit;
                string quotaUnit;
                if (IsExcelLinkSimpleBindingBlocked(row, cell, out excelUnit, out quotaUnit))
                {
                    MessageBox.Show(mainForm, BuildSimpleBindingUnitMismatchMessage(excelUnit, quotaUnit), "Excel联动", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                string fullQuantityName = FinalizeBoundLinkName(link);
        ...
        private static List<AiQuotaMatchRow> BuildAiQuotaRows(Form mainForm, SqlConnection conn, List<DataGridViewRow> rows)
        {
            ...
            foreach (DataGridViewRow row in rows ?? new List<DataGridViewRow>())
            {
                ...
                result.Add(new AiQuotaMatchRow
                {
                    ...
                    Bindable = bindable,
                    ItemNo = String.IsNullOrWhiteSpace(link.EntryCode)
                        ? ResolveChapterItemNo(conn, link.ChapterSeq, itemNoCache)
                        : link.EntryCode
                });
            }
        ...
        private static void PopulateExcelQuotaLinkLearningContext(SqlConnection conn, ExcelQuotaLink link)
        {
            if (conn == null || link == null) return;
            link.Method = SmartResolveProjectMethod(conn);
            if (String.IsNullOrWhiteSpace(link.ProjectId)) link.ProjectId = GetProjectId(conn);

            string chapterSeq = (link.ChapterSeq ?? "").Trim();
            if (chapterSeq.Length == 0) return;
            try
            {
                EnsureOpen(conn);
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        link.EntryCode = reader.IsDBNull(0) ? "" : Convert.ToString(reader.GetValue(0), CultureInfo.InvariantCulture).Trim();
                        link.EntryName = reader.IsDBNull(1) ? "" : Convert.ToString(reader.GetValue(1), CultureInfo.InvariantCulture).Trim();
                    }
                }
            }
            catch (Exception ex)
            {
                Log("Resolve Excel link learning context failed: " + ex.Message);
            }
        }

        private static void PopulateMappingFeedbackGroupProjectContext(SqlConnection conn, MappingFeedbackGroup group)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "select top 1 工程或费用项目名称 from 章节表 where 条目编号=@code";
                    cmd.Parameters.AddWithValue("@code", group.EntryCode);
                    object value = cmd.ExecuteScalar();
                    group.EntryName = value == null || value == DBNull.Value
                        ? ""
            ...
            {
                Log("Resolve mapping feedback entry context failed: " + ex.Message);
            }
        }
        ...
        // 只拆分顶层正向相加项；每一项必须只引用一个单元格。
        // 例如 F10/1000+F11/1000 -> F10,F11；F10-F11 -> F10；F10*F11 不拆分。
        private static List<string> ExtractPositiveAdditiveCellAddresses(string expression)
        {
            ...
            for (int i = 0; i <= normalized.Length; i++)
            {
                ...
                if (positive && term.Length > 0 && term.IndexOf('-') < 0)
                {
                    ...
                    if (addresses.Count == 1 && !result.Contains(addresses[0]))
                    {
        ...
        // 绑定收尾只负责读名；绑定库保存成功后再合批写 mapping-boxes，避免失败绑定污染学习库。
        private static string FinalizeBoundLinkName(ExcelQuotaLink link)
        {
            if (link == null) return "";
            Dictionary<ExcelQuotaLink, string> names = FinalizeBoundLinkNames(new List<ExcelQuotaLink> { link }, false);
            string fullName;
            return names.TryGetValue(link, out fullName) ? fullName : "";
        }

        // 批量绑定共享一次隐藏列/合并区域缓存和 ExcelSyncReadContext。
        private static Dictionary<ExcelQuotaLink, string> FinalizeBoundLinkNames(List<ExcelQuotaLink> links, bool refreshDisplayValues)
        {
            Dictionary<ExcelQuotaLink, string> result = new Dictionary<ExcelQuotaLink, string>();
            ...
            {
                if (link == null) continue;
                if (refreshDisplayValues)
                {
                    string displayValue; decimal quantity; string readError;
                    if (TryEvaluateWorkbookExpression(readContext, link.ExcelPath, link.WorksheetName, link.Expression, out displayValue, out quantity, out readError, true))
                    {
        ...
        private sealed class MappingFeedbackTarget
        {
            public string Kind;
            public string Code;
            public string Name;
        ...
        private sealed class BindingFeedbackSource
        {
        ...
        private static List<MappingFeedbackGroup> BuildBindingFeedbackGroups(Dictionary<ExcelQuotaLink, string> fullNames)
        {
            ...
            foreach (IGrouping<string, KeyValuePair<ExcelQuotaLink, string>> sourceGroup in
                (fullNames ?? new Dictionary<ExcelQuotaLink, string>())
                    .Where(pair => pair.Key != null && IsAutoMatchQuotaCode(pair.Key.QuotaCode))
                    .GroupBy(pair => BuildBindingFeedbackSourceKey(pair.Key), StringComparer.OrdinalIgnoreCase))
            {
                KeyValuePair<ExcelQuotaLink, string> first = sourceGroup.First();
                BindingFeedbackSource source = new BindingFeedbackSource();
                source.Link = sourceGroup.Select(pair => pair.Key)
            ...
            foreach (BindingFeedbackSource source in sources)
            {
                ...
                {
                    readLinks.Add(new ExcelQuotaLink { ExcelPath = source.Link.ExcelPath, WorksheetName = source.Link.WorksheetName, CellAddress = address, Expression = address });
                    AddQuantityNameReadLinks(readLinks, source.Link.ExcelPath, source.Link.WorksheetName, address, hiddenColumnCache, mergedRegionCache);
                }
            }
            ...
            foreach (BindingFeedbackSource source in sources)
            {
                ...
                for (int addressIndex = 0; addressIndex < groupCount; addressIndex++)
                {
                    ...
                    MappingFeedbackGroup group = new MappingFeedbackGroup
                    {
                        ...
                        EntryName = source.Link.EntryName ?? "",
        ...
        {
            FillGroupSheetContext(group, link, link == null ? "" : link.CellAddress);
        }

        private static void FillGroupSheetContext(MappingFeedbackGroup group, ExcelQuotaLink link, string cellAddress)
        {
        ...
        private static string FindExistingMappingBoxId(List<Dictionary<string, string>> rows, List<string> targetKeys)
        {
            List<string> expected = (targetKeys ?? new List<string>())
                ...
                .GroupBy(row => GetFlat(row, "box_id"), StringComparer.OrdinalIgnoreCase))
            {
                List<string> keys = group
                    ...
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .ToList();
                keys = keys.OrderBy(key => key, StringComparer.OrdinalIgnoreCase).ToList();
                if (keys.Count == expected.Count && keys.SequenceEqual(expected, StringComparer.OrdinalIgnoreCase))
                {
                    return group.Key;
                }
            }
        ...
        private sealed class QuickBindPanel : Form
        {
            ...
            private void BindActiveExcelCell()
            {
                ...
                BindCurrentQuota(cell.CellAddress, cell.CellAddress, cell);
            }
            ...
            {
                BindCurrentQuota(address, expression, null);
            }

            private void BindCurrentQuota(string address, string expression, ExcelCellAddress activeCell)
            {
                try
                {
                    string path = fileText.Text.Trim();
                    ...
                    string quotaUnit;
                    if (activeCell != null &&
                        String.Equals(NormalizeCellAddress(address), NormalizeCellAddress(expression), StringComparison.OrdinalIgnoreCase) &&
                        IsExcelLinkSimpleBindingBlocked(row, activeCell, out excelUnit, out quotaUnit))
                    {
                        status.Text = BuildSimpleBindingUnitMismatchMessage(excelUnit, quotaUnit);
                        return;
                    }
                    ...
                    if (activeCell != null &&
                        String.Equals(NormalizeCellAddress(expression), NormalizeCellAddress(activeCell.CellAddress), StringComparison.OrdinalIgnoreCase))
                    {
                        if (String.IsNullOrWhiteSpace(activeCell.DisplayValue))
                        {
                            evaluated = true;
                            displayValue = "";
                            readError = null;
                        ...
                        {
                            displayValue = FormatExcelInputNumber(activeCell.DisplayValue);
                    ...
                    if (!evaluated && !TryEvaluateWorkbookExpression(path, sheet, expression, out displayValue, out quantity, out readError))
                    {
