        ...
        private sealed class AgentChatWindow : Form
        {
            ...
            private static void PopulateHelpGrid(DataGridView grid)
            {
                ...
                AddHelpRow(grid, "作用范围",
                    ...
                    "多个条目或单元用顿号“、”隔开（英文逗号也兼容）；分号“；”只用于分隔多条完整命令。同一个条目编号在每个单元都存在，不写单元时默认只改当前单元。");
                AddHelpRow(grid, "编号判别",
                    "带横杠的编号当作条目；1~4 位纯数字也按条目处理；超过 4 位的纯数字按材料编号，不当作条目。",
                    "0101-01 是条目\r\n0305、0306 是条目列表\r\n1294861 是材料编号\r\nLY-21 是定额\r\nSH 是填单价代码",
                    "左侧树选中条目后，可点“插入当前条目”把条目编号填入输入框。");
                AddHelpRow(grid, "工程数量",
                    "工程数量 [条目编号] [定额编号或精确定额名称] 数字、*系数或/系数",
                    "工程数量 LY-21 100\r\n工程数量 0101-01 LY-21 *0.85\r\n工程数量 ≤25t自卸汽车运土 增运1km /2\r\n工程数量 0305 0",
                    "末尾普通数字=直接设值，带*或/=乘除原数量。定额名称完全匹配；多个完整名称用顿号“、”分隔，名称前可加条目编号进一步限定。");
                AddHelpRow(grid, "乘定额编号",
                    "定额编号 [条目编号] [定额编号] *系数 或 /系数",
                    "定额编号 0101-01 LY-21 *9\r\n定额编号 LY-21 /1.1",
                    "是在定额编号后追加乘除系数，适合软件原生缩放定额；改后通常需要在软件里手工重算。");
                AddHelpRow(grid, "单价",
                    "单价 [条目编号] [定额编号或精确定额名称] 数字、*系数或/系数",
                    "单价 SH 3500\r\n单价 0101-01 SH *1.05\r\n单价 ≤25t自卸汽车运土 增运1km 615.98 单元=04\r\n单价 ≤25t自卸汽车运土 增运1km /2 单元=所有",
                    "末尾普通数字=直接设值，带*或/=乘除原单价。名称完全匹配；普通定额的计算单价可能被主程序重算覆盖。");
                AddHelpRow(grid, "删除乘除片段",
                    "工程数量 [条目编号] [定额编号或精确定额名称] 删除*系数；删除数量 条目编号[、条目编号] 要删除的片段",
                    "工程数量 0101-01 LY-21 删除*0.85\r\n工程数量 0821-01-04-09-02 FAS 联动接入、门禁系统一键释放联动接入 删除*0 单元=所有\r\n定额编号 LY-21 删除/1.1",
                    "多个定额编号或完整名称用顿号“、”分隔；每个名称仍按完全匹配。字段里不存在该片段的行会跳过，单价不支持删除片段。");
                ...
                AddHelpRow(grid, "清空数量",
                    "清空数量 [条目编号] [定额编号]",
                    "清空数量 0101-01 LY-21\r\n清空数量 LY-21",
                    "直接设置数量使用“工程数量 … 数字”；清空数量会把数量输入和计算数量都置空。");
                AddHelpRow(grid, "替换/删除定额",
                    "替换定额 [条目编号] 原定额 新定额；删除定额 [条目编号] [定额编号]",
                    "替换定额 0101-01 LY-21 QY-100\r\n删除定额 0101-01 LY-21\r\n删除定额 LY-21",
                    "替换会保留原定额编号后面的乘除系数后缀；删除会连同相关计算缓存一起清理。");
                AddHelpRow(grid, "输入/复制/移动定额",
                    "输入定额 [条目编号] 编号=数量；复制定额 来源条目 到 目标条目；移动定额 来源条目 [定额编号、定额编号] 到 目标条目",
                    "输入定额 0101-01 LY-21=100\r\n复制定额 0101-01 到 0102-01\r\n移动定额 0305 到 0306\r\n移动定额 0305 LY-21、LY-22 到 0306 单元=04",
                    "移动定额不保留来源副本，目标只能有一个条目；省略定额编号时移动来源条目下全部定额。不写单元时只作用于当前单元。");
                ...
                AddHelpRow(grid, "多条链式执行",
                    "多条确定性指令用分号“；”隔开，一次生成预览。",
                    "工程数量 0101-01 *0.9；定额编号 0102-01 *0.9；工程数量 0103-01 100",
