    ...
    public partial class FormPanel : Form
    {
        ...
        private sealed class AgentChatWindow : Form
        {
            ...
            private static void PopulateHelpGrid(DataGridView grid)
            {
                ...
                AddHelpRow(grid, "按系数改单价",
                    "单价 [条目编号] [定额编号] *系数 或 /系数",
                    "单价 0101-01 SH *1.05\r\n单价 SH /1.1",
                    "单价按两位小数预览；仅改当前计算值，长期保留通常不如用“定额编号 *系数”。");
                AddHelpRow(grid, "按名称改单价",
                    "改单价 精确定额名称 数字 单元=单元名称；也可写 单元=所有",
                    "改单价 ≤25t自卸汽车运土 增运1km 615.98 单元=南江路泵房\r\n改单价 ≤25t自卸汽车运土 增运1km 615.98 单元=所有",
                    "按定额输入表“工程或费用项目名称”完全匹配，只改 SH/SQ/ZLF/LF/SF（含 * 或 / 系数后缀）的手填单价行；先预览，确认后执行。");
