    ...
    public partial class FormPanel : Form
    {
        ...
        private static string BuildAgentParseRequestJson(JavaScriptSerializer serializer, DeepSeekExcelMatchSettings settings, AgentContext context, string userText)
        {
            ...
            body["rules"] = new string[]
            {
                ...
                "quota_filter 填用户点名的定额编号（如 LY-21）；quota_name 填用户给出的完整定额名称并保持原文，两者只填一个。用户没限定就都留空。",
                "用户说移动定额时使用 move_quotas；target_items 必须只有一个目标条目，不能把移动解析成复制加删除。",
                "单价或工程数量后是普通数字时分别使用 set_unit_price 或 set_quantity；带*或/时使用 multiply_quantity。按名称定位时quota_name必须保持完整原文并做精确匹配。",
