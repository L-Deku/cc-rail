...
namespace RecoNet
{
    public partial class FormPanel : Form
    {
        ...
        private static string BuildAgentParseRequestJson(JavaScriptSerializer serializer, DeepSeekExcelMatchSettings settings, AgentContext context, string userText)
        {
            ...
            {
                "multiply_quantity：{\"type\":\"multiply_quantity\",\"items\":[\"条目编号\"],\"include_children\":true,\"quota_filter\":[],\"quota_name\":\"\",\"units\":[],\"target\":\"quantity\",\"operator\":\"*\",\"factor\":0.85} 按系数调整工程数量或单价；定额编号放quota_filter，精确定额名称放quota_name，两者只填一个。target=quantity或unit_price，operator为*或/。",
                "set_unit_price：{\"type\":\"set_unit_price\",\"items\":[\"@selected\"],\"quota_filter\":[\"SH\"],\"quota_name\":\"\",\"units\":[],\"value\":\"615.98\"} 把按定额编号或精确定额名称命中的单价直接设为指定数字；编号放quota_filter，名称放quota_name，两者只填一个。",
                "set_quantity：{\"type\":\"set_quantity\",\"items\":[\"条目编号\"],\"include_children\":true,\"quota_filter\":[],\"quota_name\":\"\",\"units\":[],\"value\":\"100\"} 把按条目、定额编号或精确定额名称命中的工程数量直接设为某值。",
                ...
                "move_quotas：{\"type\":\"move_quotas\",\"source_item\":\"条目编号\",\"target_items\":[\"条目编号\"],\"quota_filter\":[],\"units\":[]} 把来源条目的定额移动到唯一目标条目，不保留来源副本。",
            ...
            body["rules"] = new string[]
            {
                ...
                "quota_filter 填用户点名的定额编号（如 LY-21）；quota_name 填用户给出的完整定额名称并保持原文，多个完整名称用顿号、分隔，两者只填一个。用户没限定就都留空。",
                "用户说移动定额时使用 move_quotas；target_items 必须只有一个目标条目，不能把移动解析成复制加删除。",
                "单价或工程数量后是普通数字时分别使用 set_unit_price 或 set_quantity；带*或/时使用 multiply_quantity。按名称定位时quota_name必须保持完整原文并做精确匹配，多个名称用顿号、分隔。",
